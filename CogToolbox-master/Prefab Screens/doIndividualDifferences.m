...
%% GET PARTICIPANT #
% change here by asking session number like below -> added session no
session=input('What is study session? Test1(1), Test2(2): '); % this 
subjno = getSubjectNumber([ResultsFolder 'id'], '.txt', 1); % this function has been edited !!!

% create a file to note that this participant has been run
outfile = fopen([ResultsFolder 'session' num2str(session) '_id' num2strLZ(subjno, '%2d', 2) '.txt'], 'w'); %-> added sessio no
    ...
    if keypress ~= ExitKey   % exit key SKIPS the next task
        switch tasklist{i}
            ...
            case 'Equations' % Operation Span
                [score pct maxtime] = ospan(mainwindow, fgcolor, bgcolor, ResultsFolder, subjno, WMProcessingTime,session); % -> added session no
                fprintf(outfile, ',%2.4f,%2.4f,%2.4f', score, pct, maxtime);
                clear pct maxtime;
                load([ResultsFolder 'ResultsFile.mat'])
                SubjectData{subjno,1}=subjno;
                SubjectData{subjno,2}=score;
                SubjectData{subjno,3}=date;
                save([ResultsFolder 'ResultsFile.mat'],'SubjectData')
