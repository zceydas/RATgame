...
function [score pct maxtime] = ospan(mainwindow, fgcolor, bgcolor, datafolder, subjno, maxtime,session) % -> added session no
...
% output file
outfile=fopen([datafolder 'ospan' num2str(subjno) 'session' num2str(session) '.csv'], 'w'); % -> added session no
% header row:
